    ...
    async function loadResults() {
        try {
            // 1. DB 기준 투표 결과 가져오기
            const voteRes = await fetch(`https://votingv2-production.up.railway.app:8080/api/votes/${voteId}`, {
            ...
            for (let item of vote.items) {
                const countRes = await fetch(
                    `https://votingv2-production.up.railway.app:8080/api/votes/${voteId}/items/${item.itemId}/count`,
                    { headers: { Authorization: "Bearer " + token } }
                );
          ...
          `;
            ...
            const blockchainRes = await fetch(
                `https://votingv2-production.up.railway.app:8080/api/votes/${voteId}/results/blockchain`,
                { headers: { Authorization: "Bearer " + token } }
            );
            ...
            if (!blockchainVote.items || blockchainVote.items.length === 0) {
                // 블록체인 데이터가 없을 때
                document.getElementById("blockchain-empty").innerText = "조회된 데이터 없음2";
            } else {
